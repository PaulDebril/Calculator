...
const Calculator: React.FC = () => {
  ...
  const calculate = () => {
    try {
      ...
      setHistory(prev => [{ expr: expression, result: finalResult }, ...prev]);
    } catch {
      setPrevExpression(expression);
      setResult('Erreur');
      setIsCalculated(true);
  ...
  return (
    <div className="calculator-wrapper">
      ...
      <div className="calculator-history">
        <button className="clear-history-btn" onClick={clearHistory} aria-label="Vider l'historique">
          <BiTrash />
        </button>
        <h3>Historique</h3>
        <ul>
          {history.map((h, idx) => (
            <li key={idx} onClick={() => handleHistoryClick(h)} className="history-entry">
