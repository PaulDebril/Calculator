...
test.describe("Calculator - Soustraction", () => {
  ...
  test("soustraction de deux entiers positifs (5 - 3 = 2)", async ({
    page,
  }) => {
  ...
  test("revenir à l’expression précédente après suppression (Suppr.)", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("div").filter({ hasText: /^9-4$/ }).first(),
    ).toBeVisible();
  });
});
